...
// Check if session variable exists and is a string
if (isset($_SESSION['objuser']) && is_string($_SESSION['objuser'])) {
    $objuser = @unserialize($_SESSION['objuser']);
    
    if ($objuser === false) {
        // Handle unserialize failure (object couldn't be restored)
        echo "Failed to unserialize object.";
        // Optionally, handle the error by redirecting or logging it
    }
} else {
    // Handle session data corruption or absence
    echo "Session data is not in the correct format.";
